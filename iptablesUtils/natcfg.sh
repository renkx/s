...
turnOnNat(){
    # 开启端口转发
    echo "1. 检查端口转发状态..."
    # 直接查询内核实时参数，如果已经是 1，则跳过写入
    if [ "$(sysctl -n net.ipv4.ip_forward)" = "1" ]; then
        echo -e "   端口转发已通过外部配置开启  【跳过】"
    else
        echo -n "   端口转发开启  【成功】"
        # 如果内核没开，再尝试写入 sysctl.conf 以确保重启有效
...
addDnat(){
    ...
    sed -i --follow-symlinks "s/^$localport.*/$localport>$remotehost:$remoteport/g" $conf
    [ "$(cat $conf|grep "$localport>$remotehost:$remoteport")" = "" ]&&{
          # --- 核心修复：检查文件末尾是否有换行符，如果没有则补一个 ---
